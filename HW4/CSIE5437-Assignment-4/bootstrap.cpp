...
void BootstrapExample(uint32_t numSlots) {
    ...
    */
    ...
    std::vector<double> x1 = {2,2,2,2};
    std::vector<double> x2 = {2,2,2,2};
    ...
    Plaintext ptxt1 = cryptoContext->MakeCKKSPackedPlaintext(x1, 1, depth - 10, nullptr, numSlots);
    ...
    for(int i=0; i<5; i++){
      ciph1 = cryptoContext->EvalMult(ciph1, ciph2);
    }
    ciph1 = cryptoContext->EvalBootstrap(ciph1);
    for(int i=0; i<6; i++){
      ciph1 = cryptoContext->EvalMult(ciph1, ciph2);
    }

    std::cout << "Number of levels remaining after mul: " << depth - ciph1->GetLevel() << std::endl;

    std::cout.precision(15);
    Plaintext result;
    cryptoContext->Decrypt(keyPair.secretKey, ciph1, &result);
